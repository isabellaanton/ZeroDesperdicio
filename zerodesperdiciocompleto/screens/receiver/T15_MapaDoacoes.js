    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={[styles.header, {
        ...
        <View style={[styles.resumoCardDestaque, { width: 36, height: 36, borderRadius: 18, alignItems: 'center', justifyContent: 'center' }]}>
          <Text style={{ color: '#FFF', fontSize: 13, fontWeight: '700' }}>{DOACOES.length}</Text>
        </View>
      </View>
      ...
      {/* Card inferior */}
      <View style={{ backgroundColor: '#FDF5E6', paddingTop: 10, paddingBottom: Platform.OS === 'ios' ? 2 : 8 }}>
        <View style={{ width: 38, height: 4, borderRadius: 2, backgroundColor: 'rgba(0,0,0,0.13)', alignSelf: 'center', marginBottom: 12 }} />
          ...
          {selecionado && (
            <View style={styles.cardSelecionado}>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <View style={{ backgroundColor: '#FFF3EE', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 5 }}>
                  <Text style={{ fontSize: 11, fontWeight: '600', color: '#DA4A02' }}>⏰  {selecionado.horario}</Text>
                </View>
              </View>
              ...
              <View style={styles.cardInfoRow}>
                <View style={[styles.infoIcone, { width: 28, height: 28, borderRadius: 8, backgroundColor: '#F3F3F3' }]}>
                  <Text style={styles.cardInfoIcone}>📍</Text>
                </View>
                <Text style={{ fontSize: 13, color: '#6B6B6B', flex: 1 }}>{selecionado.endereco}</Text>
              </View>
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', alignItems: 'center', gap: 6, marginTop: 10, marginBottom: 2 }}>
          {DOACOES.map((d) => (
            <View
              key={d.id}
              style={[
                { width: 6, height: 6, borderRadius: 3, backgroundColor: 'rgba(0,0,0,0.13)' },
