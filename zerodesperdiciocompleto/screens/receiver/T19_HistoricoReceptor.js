    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.statsContainer}>
        ...
        <View style={[styles.statBox, { flex: 1, backgroundColor: theme.mode === 'dark' ? '#252525' : '#FFF3E0', borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginHorizontal: 10 }]}>
          <Text style={[styles.statNumber, { color: theme.textPrimary }]}>4.8 ⭐</Text>
        ...
        <View style={[styles.statBox, { flex: 1, backgroundColor: theme.mode === 'dark' ? '#252525' : '#FFF3E0', borderRadius: 14, paddingVertical: 14, alignItems: 'center' }]}>
          <Text style={[styles.statNumber, { color: theme.textPrimary }]}>52kg</Text>
          <Text style={[styles.statLabel, { color: theme.textSecondary }]}>recebidos</Text>
        </View>
      </View>
      ...
      <View style={{ flexDirection: 'row', backgroundColor: theme.background, paddingHorizontal: 16, paddingVertical: 12, gap: 8 }}>
        {ABAS.map((aba) => (
          <TouchableOpacity
            ...
            style={[
              styles.filtroBotao, 
              { backgroundColor: theme.cardBackground, borderColor: theme.mode === 'dark' ? '#444' : '#EEE' },
      ...
      >
        <Text style={[styles.secaoTitulo, { color: theme.textMuted }]}>{RECEBIDAS.length} doações recebidas</Text>
        ...
        {RECEBIDAS.map((item) => (
          <View key={item.id} style={[styles.cardHistorico, { backgroundColor: theme.cardBackground, borderColor: theme.mode === 'dark' ? '#333' : '#EEE', borderWidth: 1 }]}>
            <Image source={{ uri: item.imagem }} style={[styles.cardImage, { width: 100, height: 110, borderRadius: 0 }]} />
            <View style={[styles.cardContentHistorico, { padding: 12 }]}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <Text style={[styles.cardTitle, { fontWeight: '700', flex: 1, marginRight: 6, color: theme.textPrimary }]}>{item.titulo}</Text>
                
                {/* Badge de Concluído usando cores do tema */}
              ...
              <Text style={[styles.descricaoSolicitacao, { color: theme.textSecondary, marginBottom: 4 }]}>{item.detalhe}</Text>
      ...
      {/* Como o histórico não tem uma tab específica no FooterReceptor, ele não vai destacar nenhuma aba */}
