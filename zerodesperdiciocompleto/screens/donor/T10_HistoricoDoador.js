  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={[styles.header, { height: 70, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: Platform.OS === 'android' ? 12 : 8 }]}>
        <TouchableOpacity onPress={() => navigation.goBack()}><Ionicons name="arrow-back" size={22} color="#FFFFFF" /></TouchableOpacity>
        <Text style={[styles.saudacao, { fontSize: 20 }]}>Histórico</Text>
        <TouchableOpacity><Ionicons name="menu" size={24} color="#FFFFFF" /></TouchableOpacity>
      </View>

      <View style={{ flexDirection: 'row', backgroundColor: '#FFDDAE', padding: 12, gap: 8 }}>
        {['Todas', 'Este mês', 'Anteriores'].map((aba) => (
          <TouchableOpacity key={aba} style={[styles.filtroBotao, abaAtiva === aba && styles.filtroBotaoAtivo]} onPress={() => setAbaAtiva(aba)}>
          ...
          <View key={item.id} style={styles.cardHistorico}>
            ...
            <View style={[styles.cardContentHistorico, { padding: 12, flex: 1 }]}>
              ...
              <View style={{ flexDirection: 'row', gap: 12, marginTop: 6 }}>
                <Text style={{ fontSize: 11, color: '#888' }}>❤️ {item.receptor}</Text>
